...
const txstateutils = require('txstate-utils')
...
class Util {
  ...
  // mongoose objects don't work like that, so here is a convenience function
  // that does
  static dp (obj, key) {
    const usableObject = obj.toObject ? obj.toObject() : obj
    return txstateutils.get(usableObject, key)
  }
  ...
  static isNetID (netid) {
    return txstateutils.isNetID(netid)
  }
  ...
  static randomid () {
    ...
    const f = c.charAt(Math.floor(Math.random() * c.length))
    return f + this.generatestring(11)
  }
}
